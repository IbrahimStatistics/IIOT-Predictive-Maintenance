...
Implements JWT-based authentication using `PyJWT`:
- `create_access_token(username)`: issues a signed token with a 60-minute expiry
- `authenticate_user(username, password)`: checks credentials against a **temporary hardcoded test user** (`admin` / `admin123`) — explicitly flagged as a placeholder for a real users table + RBAC system, built early because retrofitting auth into an already-built API is significantly more expensive than including it from the start
- `get_current_user(token)`: a FastAPI dependency that decodes and validates the JWT on every protected request, rejecting expired or malformed tokens with a 401
...
**Design note on telemetry queries:** built with dynamically constructed `WHERE` clauses (`build_telemetry_query()`) so that `machine_id`, `start`, and `end` filters are all optional and combinable, using parameterized queries throughout (`$1`, `$2`, ...) to prevent SQL injection.

**Verified end-to-end:** after populating TimescaleDB via the simulator + consumer, both `GET /machines` and `GET /telemetry/current` were confirmed to return real, live data through the actual HTTP API — not just via direct database queries — proving the entire chain works from raw sensor recording through to an authenticated REST response.
